...
function displayNewTaskForm() {
  ...
  const project = listify.getCurrentProject(
    document.getElementById('current-view-wrapper').firstChild.innerHTML,
  );
...
function toggleTaskFavoriteStatus(taskTitle, projectTitle, star) {
  ...
  const taskObject = project.taskArray.find(
    (task) => task.taskTitle === taskTitle,
  );
  if (taskObject.isFavorite === true) {
    taskObject.isFavorite = false;
    star.classList.add('fa-regular');
    star.classList.remove('fa-solid');
  } else {
    taskObject.isFavorite = true;
...
function deleteCurrentProject(e) {
  ...
  const index = listify.projectArray.indexOf(project);
  // Using project object and index, remove
  listify.projectArray.splice(index, 1);
  // Redisplay the side panel with removed project gone
  listify.displayAllCurrentProjectsSidebar();
